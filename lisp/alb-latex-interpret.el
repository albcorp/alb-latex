;;;
;;; AlbAUCTeXParser/alb-latex-interpret.el
;;;
...
;;;     MA 02111-1307, USA.
;;;
;;;   - Functions to interpret parse trees yielded by the `alb-LaTeX-parse-*'
;;;     functions.
;;;
;;;   - The interpreter traverses the parse tree whilst collecting a single
;;;     value: the file list.  This is a possibly incomplete list of visited
;;;     files.  The first element is always the master filename, the second
;;;     element is always the current filename, but other filenames need not
;;;     be passed in the list.  All other actions are performed as side
;;;     effects of this traversal.
;;;
;;;   - The interpreter is composed of mutually recursive functions: the
;;;     `alb-LaTeX-interpret' function recurs on the elements of a parse tree
;;;     forest, as generated by an `alb-LaTeX-collect-*' function; while
;;;     functions named `alb-LaTeX-interpret-*' process single parse trees as
;;;     generated by an `alb-LaTeX-parse-*' function.
;;;
;;;     The two forms are related by a context argument, which is used to
;;;     associate parse trees with specialised interpreter functions.  Thus,
;;;     the basic interpreter can be extended to interpret particular forms.
;;;
;;;   - Example: the interpreter in "alb-graphicx.el" generates a sequence of
;;;     LaTeX source files by progressive updates to the current buffer.  The
;;;     files are used to generate PDF versions of PSfragged EPS graphics.
;;;     The filelist is used to transmit a dependency list for the current
;;;     buffer state, and the buffer is compiled only when the target is older
;;;     than a file on the dependency list.
;;;
